...
base_url = 'https://axcrmservice.dynu.net:9501/hash2phone/map_hash_num.php'  # URL to hash2phone matcher
...
while 1:
    ...
    if len(devs):
        for i in range(len(devs)):
            print("------------------------------------------------------------------------------")
            my_str = devs[i]["hashEmail"]
            n = 64
            my_list = [my_str[idx:idx + n] for idx in range(0, len(my_str), n)]
            finalEmail = my_list if len(my_list) > 1  else my_str
            
            print("Phone: \033[92m{}\033[0m \nip Address: \033[92m{}\033[0m \nEmail: \033[92m{}\033[0m \nFileName: \033[92m{}\033[0m \nTime: \033[92m{}\033[0m".format(devs[i]["hashPhone"], devs[i]["ip"] , finalEmail, devs[i]["FileName"], devs[i]["Time"]))
            print("------------------------------------------------------------------------------")
            del devs[i]
        
#            print("Dev for dev in devs",dev)
#            print("result", results)
#            if dev["phone"] not in results.keys():
#                if dev["hashPhone"] not in results.keys():
#                    if args.check_hash:
...
#                                                          dev["phone"]))
#                    else:
#                        print("Someone with phone number hash \033[92m{}\033[0m has tried to use AirDrop \n ip Address \033[92m{}\033[0m".format(
#                            dev["hash"], dev["ip"]))
#
#                elif dev["hashEmail"] not in results.keys():
